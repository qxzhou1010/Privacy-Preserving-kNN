...
def DataUpload(trainData):
    ...
    for i in range(trainDataNums):
        encOftrainData[i] = mvhe0.SpecialEncrypt(M,e,trainData[i])
    M = np.array(M, dtype=np.float64)
    MPinv = np.linalg.pinv(M)
    HA = I.dot(MPinv)
    H = HA.T.dot(HA)
    M = mvhe0.SpecialKeySwitchMatrix(I, T, Pm, A)
    return encOftrainData,M,H,e
...
def userEnc( M,e,queryPoint):
    return mvhe0.SpecialEncrypt(M,e,queryPoint)
...
if __name__ == '__main__':
    timeStar = clock()
    trainDataOfEnc, M, H,e = DataUpload(trainData)
    ...
    for i in range(testNums):
        encTestData[i] = userEnc(M, e, testData[i])
