...
def QueryGen(queryDataSet, S, T_cols):
    ...
    # newT = mvhe.getRandomMatrix(queryPoints, T_cols, vhe.tBound)  # 产生新随机矩阵T
    # newS = vhe.getSecretKey(newT)  # 新密钥
    # GS = G.dot(S)
    # M = vhe.KeySwicthMatrix(GS, newT)  # 密钥转换矩阵
    # # M send to the cloud,newS keep Secretly
    # return M, newS
...
def knn(decDist, kPoints):
    testPoints, trainPoints = decDist.shape  # 训练数据的点数，获取现在应该为列数,即是需要进行排序的值的个数
    labelPreMat = np.zeros((testPoints, 1), dtype=object)  # 由于我要返回的是一个labelPreMat，所以要把每行的向量单独列出来


    all_dist = np.zeros((trainPoints, 2))  # 存样本序号 和 对应距离
    for j in range(testPoints):                 #每次取对一个点，对其分类判断
        singleDecPoint = np.copy(decDist[j])    #
        singleDecPoint.resize((trainPoints, 1))
        for i in range(trainPoints):
            all_dist[i][0] = i
            all_dist[i][1] = singleDecPoint[i][0]
...
if __name__ == '__main__':
    ...
    M, newS = QueryGen(testData, S, T_cols)
    encDist = ScoreCalculate(trainDataOfEnc, M, T_cols)
    labelPreMat = SCMC(encDist, newS, kPoints=3)
